...
async function fetchProductsFromSvelteData(pageUrl) {
    ...
    for (const candidate of candidates) {
        try {
            ...
            const products = discoveredProducts.length > 0 ? formatProducts(discoveredProducts) : [];
    ...
    try {
        ...
        if (directProductsArray) {
            try {
                ...
                if (Array.isArray(parsed) && parsed.length > 0) {
                    const formattedProducts = formatProducts(parsed);
        ...
        while ((match = scriptRegex.exec(html)) !== null) {
            ...
            if (svelteProducts.length > 0) {
                return formatProducts(svelteProducts);
            }
        }
