class JumiaSKUFinder {
    ...
    async fetchProducts(url) {
        // Try CORS proxy first to bypass local environment limitations
        try {
            ...
            // Fallback to Vercel API if available
            try {
    ...
    extractMetaFromHTML(html) {
        ...
        // Try to find window.__STORE__ pattern for meta
        const storePatterns = [
            /window\.__STORE__\s*=\s*({[\s\S]*?});\s*<\/script/,
            /window\.__STORE__\s*=\s*({[\s\S]*?});/,
            /__STORE__\s*=\s*({[\s\S]*?});/
        ...
        for (const pattern of storePatterns) {
            ...
            if (storeMatch) {
                try {
                    const storeData = JSON.parse(storeMatch[1]);
                    // Look for meta fields in storeData
                    if (storeData.products && storeData.totalProducts) {
                        meta.totalProducts = storeData.totalProducts;
                        meta.totalPages = storeData.totalPages;
                        meta.pageSize = storeData.pageSize;
                        meta.page = storeData.page;
                    }
                } catch (e) {}
            }
        }
