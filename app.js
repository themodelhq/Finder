class JumiaSKUFinder {
    ...
    extractProductsFromHTML(html) {
        try {
            ...
            // Method 2: Look for products in script tags
            ...
            while ((scriptMatch = scriptRegex.exec(html)) !== null) {
                ...
                const productsMatch = scriptContent.match(/"products"\s*:\s*\[({[\s\S]*?})\]/);
                
                if (productsMatch) {
                    try {
                        const jsonStr = `{"products":[${productsMatch[1]}]}`;
                        const parsed = JSON.parse(jsonStr);
                        
                        if (parsed.products && Array.isArray(parsed.products)) {
                            return this.formatProducts(parsed.products);
                        }
                    } catch (e) {
                        // Try to find the complete products array
                        const fullMatch = scriptContent.match(/"products"\s*:\s*(\[[^\]]*\])/);
                        if (fullMatch) {
                            try {
                                const products = JSON.parse(fullMatch[1]);
    ...
    buildSellerFilter() {
        const sellers = [...new Set(this.originalData.map(p => {
            return p.sellerName || (p.shopGlobal?.name) || 'N/A';
        }).filter(s => s && s !== 'N/A'))];
        sellers.sort();
    ...
    createProductCard(product) {
        ...
        const sellerName = product.sellerName || (product.shopGlobal?.name) || '';
    ...
    applyFilters() {
        ...
        if (sellerValue !== '0') {
            filtered = filtered.filter(p => {
                const sellerName = p.sellerName || (p.shopGlobal?.name) || 'N/A';
    ...
    async fetchExportData() {
        ...
        try {
            // Data is already fetched, just build the table
    ...
    buildExportTable() {
        ...
        // Build header
        const headers = ['SKU', 'Name', 'Brand', 'Category', 'Rating', 'Image', 'URL', 'Old Price', 'New Price', 'Stock'];
