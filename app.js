class JumiaSKUFinder {
    ...
    async fetchProducts(url) {
        ...
        try {
            ...
            console.log('API returned empty results, trying SvelteKit data fallback...');
            throw new Error('Empty results from API');
            
        } catch (apiError) {
            console.error('Vercel API error:', apiError);
            
            // Try SvelteKit __data.json endpoint directly (Jumia uses SvelteKit)
            try {
                const svelteProducts = await this.fetchProductsFromSvelteData(url);
                if (svelteProducts && svelteProducts.length > 0) {
                    console.log(`Fetched ${svelteProducts.length} products via SvelteKit data endpoint`);
                    return svelteProducts;
                }
            } catch (svelteError) {
                console.log('SvelteKit data fetch failed:', svelteError);
            }

            // Fallback to CORS proxies (single attempt, no duplicate)
            return await this.fetchProductsWithCORS(url);
        }
    }

    async fetchProductsFromSvelteData(pageUrl) {
        const candidates = this.buildSvelteDataUrls(pageUrl);
        for (const candidate of candidates) {
            try {
                ...
                const meta = this.extractMetaFromObject(payload);
                if (meta && Object.keys(meta).length > 0) {
                    this.lastFetchMeta = this.mergeMeta(this.lastFetchMeta, meta);
                }
                const discovered = this.findProductArrays(payload);
