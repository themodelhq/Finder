class JumiaSKUFinder {
    ...
    async fetchSellerNameFromProductPage(productUrl) {
        try {
            ...
            // Method 2: Extract from simpler seller card pattern
            const simpleSellerMatch = html.match(/<h2[^>]*>Seller Information<\/h2>[\s\S]*?<p[^>]*>(.*?)<\/p>/i);
            if (simpleSellerMatch && simpleSellerMatch[1]) {
                const name = simpleSellerMatch[1].trim();
                if (name && !name.includes('<') && name.length > 0 && name.length < 100) {
                    return name;
                }
            }
            
            // Method 3: Extract from Product Line specification
            ...
            // Method 4: Extract from any seller link pattern
            ...
            // Method 5: Extract from window.__STORE__ data (most reliable if present)
            const storeMatch = html.match(/window\.__STORE__\s*=\s*({[\s\S]*?});/);
            if (storeMatch) {
                try {
                    const storeData = JSON.parse(storeMatch[1]);
                    
                    // Check googleAds targeting seller
                    if (storeData.googleAds?.targeting?.seller?.length > 0) {
                        return storeData.googleAds.targeting.seller[0];
                    }
                    
                    // Check product seller data
                    ...
                    // Check shopGlobal
                    if (storeData.shopGlobal?.name) return storeData.shopGlobal.name;
                    if (storeData.shopGlobal?.displayName) return storeData.shopGlobal.displayName;
                } catch (e) {
                    console.log('Failed to parse __STORE__ for seller:', e.message);
                }
            }
            
            // Method 6: Look for seller info in structured data (ld+json)
    ...
    buildSellerFilterAfterFetch() {
        // Only use actual seller names (not brands) after fetching
        const sellers = [...new Set(this.originalData.map(p => {
            // Only include if it's an actual seller name (not just the brand)
            ...
            // Exclude if it's the same as brand or N/A
            if (sellerName && sellerName !== 'N/A' && sellerName !== p.brand) {
                return sellerName;
            }
            return null;
        }).filter(s => s !== null))];
    ...
    async fetchExportData() {
        ...
        try {
            // Fetch seller names in parallel batches for speed
            ...
            for (let i = 0; i < this.data.length; i += batchSize) {
                const batch = this.data.slice(i, Math.min(i + batchSize, this.data.length));
                ...
                await Promise.all(batch.map(async (product, batchIndex) => {
                    ...
                    if (!currentSellerName || currentSellerName === product.brand || currentSellerName === 'N/A') {
                        ...
                        if (sellerName && sellerName !== product.brand) {
                            this.data[dataIndex].sellerName = sellerName;
                            successCount++;
                            console.log(`✓ Found seller: ${sellerName} for SKU: ${product.sku}`);
                        } else {
                            // Fallback to existing data (but not brand)
                            this.data[dataIndex].sellerName = product.shopGlobal?.name || 'N/A';
                            console.log(`✗ No seller found for SKU: ${product.sku}, using fallback: ${this.data[dataIndex].sellerName}`);
                        }
                    } else {
                        // Already has seller name
                        this.data[dataIndex].sellerName = currentSellerName;
                        successCount++;
            ...
            // Rebuild seller filter with only actual seller names (not brands)
            this.buildSellerFilterAfterFetch();
